...
Option : --config <fichier>   autre fichier de configuration (défaut : config.local.json)
`;

function lireArgs(argv) {
  const args = { commande: null, adoption: false, config: join(RACINE, "config.local.json") };
...
async function main() {
  ...
  const store = await creerStore({ ...config.firebase, espace: config.espace });
  const contexte = { notion, store, ui: creerUI(), config, maintenant: Date.now() };
